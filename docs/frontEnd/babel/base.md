...
`babel-polyfill`的作用是：提供ES6内置方法和函数转化垫片(shim)，如Map,Set,Promise等。
...
babel-polyfill 若直接使用，主要有两个缺点：

1. 使用 babel-polyfill 会导致打出来的包变大，且不能按需引入。
...
2. 问题2：这是 babel-polyfill 的特性造成的，不可能解决，除非不用它。
